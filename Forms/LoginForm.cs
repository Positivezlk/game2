...
{
 readonly TextBox login=Ui.TextBox(); readonly TextBox password=Ui.TextBox(){UseSystemPasswordChar=true}; public User? CurrentUser{get;private set;}
 ...
 void DoLogin(){var user=new AuthService().Login(login.Text.Trim(),password.Text); if(user==null){MessageBox.Show("Неверный логин или пароль","CertDesk",MessageBoxButtons.OK,MessageBoxIcon.Warning);return;} CurrentUser=user; DialogResult=DialogResult.OK; Close();}
}
